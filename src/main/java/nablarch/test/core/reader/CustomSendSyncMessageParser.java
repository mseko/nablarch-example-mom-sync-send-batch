...
import nablarch.test.core.reader.*;
...
public class CustomSendSyncMessageParser extends CustomMessageParser {
     ...
     */
    public CustomSendSyncMessageParser(TestDataReader reader, List<TestDataInterpreter> interpreters, DataType targetType) {
    ...
    public static enum ErrorMode {
        ...
        public static boolean isErrorMode(String mode) {
            for (SendSyncMessageParser.ErrorMode e : SendSyncMessageParser.ErrorMode.values()) {
        ...
        return new FixedLengthFileParser(reader, interpreters, targetType) {
            ...
            protected void onReadingValues(List<String> line) {
                ...
                if (temp.size() > ERROR_MODE_COLUMN_NUMBER) {
                    ...
                    if (SendSyncMessageParser.ErrorMode.isErrorMode(errorMode)) {
                        // エラー系のテストの場合
